...
\prop{Non-expansiveness of the proximal operator}{
...
\thm[co-coercivity]{Co-coercivity}{
	...
	\begin{enumerate}
		\item Quadratic upper bound (compare with Proposition~\ref{prop:convexity-diff}):
		\begin{equation}
...
\ex{}{
If we are trying to learn a regression model $\omega = (\hat z, \hat y)$ would be the observations, the model would be $y=mz+q$, with parameter $x=(m,q)$. 
Then, the loss function for the sample $(\hat z,\hat y)$ is then
\begin{equation}
...
\thm{}{
	Let $g:\bbR^d\to \bbR$ be smooth, and $f\in C^1(\bbR^d)$ be convex, smooth, and such that $\nabla f$ is Lipschitz with constant $L>0$. 
...
\[
    x_{k+1} = S_{\alpha t}\left(x_k - 2t A^\top (A x_k - b)\right),
\]
where $S_{\alpha t}$ is the soft-thresholding operator \eqref{eq:soft-thresholding} with parameter $\lambda t$. 
}
...
\thm{Dual proximal gradient}{
	...
	In particular, if \eqref{eq:f+gA-dual} admits a unique minimizer $x^\star$, and $\alpha_n = \alpha <\gamma /\|A\|$. 
}
